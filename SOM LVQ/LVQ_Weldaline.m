...
testinp = data(trainttl+1:rows,1:columns);
traintk = data(1:trainttl,columns+1); % untuk training
testtk = data(trainttl+1:rows,columns+1); % untuk testing

a = 0.5;            % laju pembelajaran
c = 0.2;              	% konstanta penurunan laju pembelajaran
d = zeros(y,1);         % jarak euclidean
epoch = 0;              
benar = 0;       % menghitung jumlah yang benar
...
while true
    ...
    for count = 1:trainttl
        ...
        if indeks == traintk(count)
            for x = 1:x
                vp(indeks,x) = vp(indeks,x) + (a * (traininp(count,x) - vp(indeks,x))); % hitung Wp baru
            end
        end
